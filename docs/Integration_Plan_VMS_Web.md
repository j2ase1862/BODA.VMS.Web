...
- [x] **Phase 7 / Stage 3 (SignalR 푸시) — C5** — Web 측 `Hubs/VmsPublicHub.cs` 익명 Hub + `/hubs/vms-public` 매핑. `/api/parameters/results` 에서 `IHubContext<VmsPublicHub>` 주입 → `WorkOrderUpdated` / `WorkOrderCompleted` broadcast. VMS 측 `Microsoft.AspNetCore.SignalR.Client` 8.0.0 + `VmsHubClient` 서비스 (WithAutomaticReconnect + self-signed cert 우회 + IAsyncDisposable). MainViewModel 가 두 이벤트 소스(응답 파싱 + SignalR) 구독, `_lastCompletedNotifiedWoId` 가드로 중복 다이얼로그 차단. (2026-05-21, VMS 에이전트가 양쪽 작성)
- [x] **D8 — VMS 자체 검사 히스토리** — In-Memory 순환 버퍼 `RecentInspectionsService` (MaxItems=200, Singleton, INotifyPropertyChanged) + `InspectionRecord` DTO. `InspectionService.CollectAndUploadParameterResults` 에서 업로드 전에 push (Web 끊겨도 기록). 사이드 패널 새 섹션 "Recent Inspections" — 3-cell 통계 + Pass rate + ListBox (최신 위, OK/NG 컬러 뱃지, NG 코드, 풀 타임스탬프 ToolTip) + Clear 버튼. (2026-05-22, VMS 에이전트)
- [x] **D10 — Operator 등급(Role) 기반 메뉴 가시성** — Web 측 `Operator.Role` 필드 + 마이그레이션 (PRAGMA + ALTER TABLE ADD COLUMN Role DEFAULT 'Operator') + `OperatorDto`/`OperatorUpsertDto`/`OperatorSessionDto` 에 Role 전파 + `OperatorService.NormalizeRole` 정규화 + `OperatorFormDialog.razor` 에 Role MudSelect. VMS 측 `OperatorSessionDto.Role` + `OperatorRoles` 상수 + MainViewModel `CurrentOperatorRole` / `CanLeadOrAbove` / `CanSupervisor` 계산 프로퍼티 + 헤더 Operator chip 에 Role 뱃지 (Lead=파랑/Supervisor=주황/Operator=숨김) + 사이드 패널 External Tools 섹션을 Supervisor 만 표시. Standalone (OperatorAuthService null) 환경은 기존 동작 유지. (2026-05-22, VMS 에이전트가 양쪽 작성)
...
- [x] **B4 — 헤더 WO 칩 ProgressBar 시각화** — `SelectedWorkOrderProgressPercent` / `HasSelectedWorkOrderProgress` 계산 프로퍼티 + ProgressBar Width 80 Height 6 with 커스텀 Template (다크 #1A1A1C 트랙 + #14B8A6 인디케이터, CornerRadius 3). OnWorkOrderProgressed 에서 알림 발생으로 결과 업로드 응답마다 즉시 갱신. (2026-05-21, VMS 에이전트)
- [x] **C6 — 업로드 큐/자동 재시도** — `ParameterSyncService` 에 disk queue (`%LocalAppData%/BODA VISION AI/upload_queue/{timestamp}_{guid}.json`) + 5초 주기 retry timer. UploadResultsAsync 분리 (TrySendAsync + EnqueueFailed). 첫 실패 시 즉시 중단 — 순서 보존 + 서버 다운 시 스팸 방지. PendingUploadCount 프로퍼티 노출. MainViewModel.OnWorkOrderCompletedFromServer 가드: stale 한 WO completed 가 다른 WO unselect 못하도록 Id 일치 검사. (2026-05-21, VMS 에이전트)

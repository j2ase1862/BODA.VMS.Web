...
using (var scope = app.Services.CreateScope())
{
    ...
    await db.Database.ExecuteSqlRawAsync(@"
        CREATE TABLE IF NOT EXISTS ""WorkOrders"" (
            ...
            ""CompletionBasis""   TEXT NOT NULL DEFAULT 'Produced',
            ""PlannedStartAt""    TEXT,
            ""ActualStartAt""     TEXT,
            ""ActualEndAt""       TEXT,
            ""Note""              TEXT,
    ...
    // 기존 DB 에 완료 기준 컬럼 보강 — 기본 'Produced' (신규 설치도 동일).
    // 양품 기준은 WO 생성/편집에서 명시적으로 선택 (2026-08-19 opt-in 정책).
    ...
    // 디폴트 admin/admin 하드코딩 제거. 시드/강제 리셋/불일치 진단은
    // AdminAccountBootstrap 참조 (Initial:AdminUsername / AdminPassword /
    // ForceAdminPasswordReset). 미설정 + DB 에 admin 도 없음 → 첫 가동 차단
    // (RequireExplicit 패턴, JWT Key 와 동일 정책).
    await AdminAccountBootstrap.EnsureAdminAsync(db, builder.Configuration, app.Logger);
